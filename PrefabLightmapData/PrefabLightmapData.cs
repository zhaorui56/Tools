...
public class PrefabLightmapData : MonoBehaviour
{
    ...
    struct RendererInfo
    {
        public RendererInfo(Renderer re, int i, Vector4 v4)
        {
            renderer = re;
            lightmapIndex = i;
            lightmapOffsetScale = v4;
        }
    ...
    private static List<RendererInfo> _renderList = new List<RendererInfo>();

    static void FindRender(Transform trans)
    {
        foreach (Transform per in trans)
        {
            if (per.childCount == 0)
            {
                Renderer render = per.GetComponent<Renderer>();
                if (render != null)
                {
                    if (render.gameObject.isStatic)
                        _renderList.Add(new RendererInfo(render, 0, Vector4.zero));
                }
            }
            else
            {
    ...
    [MenuItem("Assets/Check RenderInfo")]
    static void CheckRenderInfo()
    {
        PrefabLightmapData[] prefabs = FindObjectsOfType<PrefabLightmapData>();

        foreach (var instance in prefabs)
        {
            Object objSelected = instance.gameObject;
            if (objSelected != null)
            {
                Transform trans = (objSelected as GameObject).transform;
                if (trans != null)
                {
                    FindRender(trans);
                }
            }
            instance.m_RendererInfo = _renderList.ToArray();
        }
    }
